...
const requestSchema = z.object({
  ...
  country: z.string().trim().toUpperCase().regex(/^(ALL|[A-Z]{2})$/, 'country deve ser ALL ou um código ISO de 2 letras'),
}).strict();
  ...
  try {
    ...
    const product = await prisma.productResearch.upsert({
      ...
      create: {
        ...
        network: 'Meta_Google_Scout',
        productType: 'PROPRIETARY_LOW_TICKET',
        vertical: result.idea.vertical,
        score: result.idea.potentialScore,
        source: 'trend-scout',
        status: 'novo',
        chosenKeyword: parsed.data.niche,
        summary: result.idea.vslHook,
        tags: [parsed.data.niche, parsed.data.country, 'trend-scout'],
        strategy: {
          pricing: result.idea.pricing,
          vslHook: result.idea.vslHook,
          leadMagnet: result.idea.leadMagnet,
          bonusSuggested: result.idea.bonusSuggested,
    ...
    console.error('[trend-scout] POST error:', message);
    return NextResponse.json({ error: 'Erro ao gerar produto Trend-Scout', details: message }, { status });
  }
}
