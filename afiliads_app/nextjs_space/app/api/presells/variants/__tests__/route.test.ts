...
function request(body: unknown) {
  return new NextRequest('http://localhost/api/presells/variants', { method: 'POST', body: JSON.stringify(body) });
}

describe('POST /api/presells/variants', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(getServerSession).mockResolvedValue({ user: { id: 'user-1' } } as never);
  });

  it('rejeita campaignId de outro usuário antes de gerar variantes', async () => {
