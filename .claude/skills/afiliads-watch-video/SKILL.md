...
- **X / IG / TikTok video**: URL — defers to `afiliads-social-fetch` for metadata, uses yt-dlp for the file
...
|---|---|---|
| `/afiliads-watch-video <url>` | **transcript** (default) | Clean text, metadata, optional chapters |
| `/afiliads-watch-video <url> transcript` | transcript | Same as default |
| `/afiliads-watch-video <url> visual` | visual | Transcript + frames at intervals + Claude vision pass identifying key moments |
| `/afiliads-watch-video <url> multimodal` | multimodal | Native video to Gemini (if `$GEMINI_API_KEY`), else dense Claude vision frame-by-frame |
...
| X / IG / TikTok | Defer to `afiliads-social-fetch` for metadata, yt-dlp for file | No | Short-form |
| Local file | n/a | n/a | Drop a path |

## Composes with

- `afiliads-social-fetch` — for X/IG/TikTok URL metadata (engagement, author, replies) before video processing
...
- **`skillify from-video`** — primary use case for `visual` mode on process recordings. the user records themselves doing a workflow (Loom/screen-share), this skill extracts transcript + key visual moments, then `afiliads-skillify` synthesizes the workflow into a SKILL.md. "Record once, AI converts to skill."
