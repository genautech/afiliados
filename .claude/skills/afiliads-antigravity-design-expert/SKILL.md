--- 
name: afiliads-antigravity-design-expert
description: Core UI/UX engineering skill for building highly interactive, spatial, weightless, and glassmorphism-based web interfaces using GSAP, 3D CSS, and Framer Motion.
risk: safe
source: community
...
- The design should lean on GSAP, 3D CSS transforms, Framer Motion, or React-based 3D presentation patterns.
...
- **Animation:** GSAP (GreenSock) + ScrollTrigger for scroll-linked motion, Framer Motion (`motion/react`)
- **3D Elements:** React Three Fiber (R3F), Three.js, or CSS 3D Transforms (`rotateX`, `rotateY`, `perspective`)
...
- **Scroll Hijacking (Tasteful):** Use GSAP ScrollTrigger or Viewport triggers to make elements float into view from the Y-axis with slight rotation as the user scrolls.
