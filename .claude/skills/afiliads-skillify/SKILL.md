...
| **from-video** | User recorded a Loom / Zoom / screen-share. Calls `afiliads-watch-video` in visual mode → transcript + key visual moments → SKILL.md. Best for visual/UI-heavy workflows. |
...
Call those directly when in doubt about format. `afiliads-skillify` orchestrates *your* workflow — which repo, which conventions, which cross-references. Format expertise lives upstream.
...
2. Call `afiliads-watch-video <url> visual` — get transcript + key visual moments + summary.
...
**from-scratch:**
Interactive Q&A:
1. Name (kebab-case, verb-noun preferred — matches `afiliads-watch-video`, `read-book`, `paste`)
2. One-line purpose
...
| **Adapt** | Tool defaults, paths, voice, naming, opinions | Their video tool → your `afiliads-watch-video`, their kanban → your `pm`, their voice → your voice |
...
**Adapt:**
- Naming: rename to verb-noun if not (matches `afiliads-watch-video`, `read-book`, `paste`)
- Tool swaps: their generic kanban → `pm`, their video tool → `afiliads-watch-video`, their note system → `second-brain`, their decision framework → `decide`
...
- **`afiliads-watch-video`** — load-bearing for CREATE / from-video. Visual-mode output (transcript + key visual moments + summary) is the input for skill synthesis. Always call in `visual` mode for process recordings — UI state matters as much as words.
...
- **Verb-noun naming** where possible (matches `afiliads-watch-video`, `read-book`, `paste`). Single-word noun names are fine for distinctive concepts (`decide`, `pm`, `paste`, `afiliads-skillify`).
