...
**Atualização em 2026-08-25 — B1/B4/B7 encerradas:** o deadlock de keywords (B1), a semântica explícita de lançamento (B4) e os gates server-side dos Passos 7/8 (B7) estão concluídos e aceitos. A Tarefa 10/B2/B6 permanece fechada; não reabrir sem evidência de regressão. O próximo foco é PAGES-1, seguido de PAGES-3, que depende do seletor de tipo de página.

**Estado em 2026-07-29 (~17:30h) — Sessão Anti-Gravity (Tarefa 10AF - Fechamento final da fundação):** Concluídas as correções obrigatórias para P1 e P2 exigidas em 10AF. (1) O `readiness` agora opera com modos explícitos (`PREPARE` ou `SCHEDULE`). O modo `PREPARE` trata a lacuna 10B (URL não verificada contra checklist persistido) apenas como *warning* caso a URL seja HTTPS válida, não bloqueando a veiculação, enquanto `SCHEDULE` exigirá (futuramente) validação restrita com falha bloqueante; a rota `/api/google-ads/create` foi atualizada para chamar explicitamente `PREPARE`. Adicionados testes de mock para a rota de `create`, exercitando o pipeline até `createGoogleCampaign` no caso válido, e bloqueio na falha de readiness. (2) O schema do PATCH foi refinado, extraindo `languages` e forçando limites estritos e não-negativos para budget e URLs; substituídos os "any" de readiness por interfaces mínimas (`ReadinessCampaign`, `ReadinessKeyword`); e o `AuthorizationContext` ajustado para refletir o `userId` autenticado, cobrindo testes de resourceOwnerUserId idêntico. Testados e removidos resíduos de whitespaces. Os testes (229/229), TSC e Build foram reproduzidos. Tarefas 10B/10C não iniciadas.

**Estado em 2026-07-29 (~16:30h) — Sessão Anti-Gravity (Tarefa 10AR - Correções da auditoria de fundações):** Implementadas as correções apontadas na auditoria 10A. (1) PATCH Wizard: schema de validação alinhado com tipos exatos do Prisma (strings para loopInterval/loopAgents/testDuration) e payloads reais de saveCampaign/saveLoopConfig inclusos nas fixtures; adicionado schema `platform`. Exceção de "wizard reset" removida para forçar transições de status legais. (2) IDOR productResearchId resolvido: handler checa `id` + `userId` no banco, retornando 404 sem update se não pertencer. Adicionado teste completo de HTTP handlers via mock (`app/api/campaigns/[id]/__tests__/route.test.ts`). (3) Readiness: url HTTPS aplicada estritamente via `new URL()` (rejeitando http/javascript/credenciais). Inserido erro bloqueante explicitando lacuna estrutural para 10B na garantia da URL (vínculo do checklist). Adicionada validação de cast estrito para MatchTypes (EXACT/PHRASE/BROAD). (4) Auth Contract: `MutationAuthorizationSchema` enrijecido com limites de string, `.strict()`, regex em idempotencyKey e validação de `currentUserId` contra `expectedUserId`. Testes e builds `npx tsc --noEmit` & `npm run build` passaram (225 testes verdes). Nenhuma rota de experiments ou mutation no Google Ads criada.
...
**Nota (Codex):** A Tarefa 10B de orquestração de experimentos (`lib/google-ads-experiments/orchestration.ts`) foi implementada e validada pelo Codex nos commits `7f451fc` a `de56b6a` (saga, idempotência, checkpoints, reconciliação fail-closed, DTOs seguros e sync) com 112 testes de integração/unidade, TSC e build. Nenhuma chamada real ao Google Ads/banco. Esta entrega se refere à idempotência do *lifecycle* de experiments, enquanto a 'lacuna 10B' anterior se refere à verificação persistida de URLs. É necessário separar os commits para merge. 
...
**Nota (Codex - Tarefa 10C - Rotas e Gates de Mutação):** Implementação concluída. O Codex implementou PATCH de campanhas com allowlist estrita, ownership por id + userId, validação HTTPS, limites e normalização de datas. Tratamento fail-closed para JSON inválido e sessões sem userId. Rota de criação Google Ads protegida por autorização explícita, mutation guard e retry idempotente, evitando duplicação. Rotas de Experiments 10C existentes auditadas e cobertas por mocks. Foram adicionados novos testes adversariais para URLs, DateTime, status inválido e retry. Todos os 522 testes passaram, TypeScript sem erros, build Next.js concluído. Nenhuma chamada real ao Google Ads ou mutação em banco de dados foi executada.

**Análise (Codex - Abrangência de Rotas Reais da Tarefa 10/10C):** As principais rotas de mutação para Campanhas (criação, PATCH, decisões, ownership), Google Ads (create e sync/push com autorização/guard), Experiments (setup, detalhe, schedule, actions e sync) e Checklists (leitura, verificação, correções com ownership) estão cobertas. Os gates B2/B6 estão conectados. Rotas como 'launch/activate campaign' (pertence a B4), deadlock de keywords (B1) e novos gates do Wizard 7/8 (B7) não devem ser adicionadas agora. A rota `/api/loop/run` é automação futura. `DELETE /api/campaigns/[id]` pode merecer política de confirmação futura. A abrangência de "Rotas Reais" é suficiente para a Tarefa 10C.

**Atualização mesma sessão (~15h) — Tarefas 9R, 9R2 e 9F concluídas (hashes 897264a e docs):** Auditoria final documental e hardening extremo do módulo de relatórios (`lib/google-ads/experiment-reporting.ts`).
(1) Injection em Fallback GAQL: a query de fallback agora valida estritamente a sintaxe do `resourceName` contra uma regex de campanha para falhar rápido e evitar inputs com aspas maliciosas.
(2) Sanitização OOM e limites (9F): `sanitizeSourcePayload` agora limita ativamente o comprimento das strings campo-a-campo *antes* de injetá-las no objeto seguro, barrando o consumo de memória antes do `JSON.stringify`.
(3) Numéricos Estritos (9F): `parseStrictNumber` usa regex rigorosa para rejeitar lixo numérico invisível (`0x10`, `0b10`, ou strings com espaços internos) que o `Number()` nativo aceitaria incorretamente.
(4) Alvos Inválidos: `targetClicks` agora lança erro (falha fechada) para strings, `NaN`, e números negativos/fracionados, sem mutar o alvo original.
