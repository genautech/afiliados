...
1. **O vendor pode proibir o canal inteiro OU só o brand bidding — nunca confunda os dois.** Antes de montar QUALQUER campanha, busque e leia a página real de afiliados/advertising rules do vendor procurando por "search", "adwords", "google", "brand", "trademark" — não confie em texto de marketing ("converte bem no Google!"), que quase sempre significa Google Display/YouTube, não Search. Duas restrições bem diferentes aparecem nesses documentos: (a) proibição do canal inteiro ("does not allow any traffic from Google unless it is Display or YouTube" — aí sim, não roda Search de jeito nenhum); (b) proibição de brand bidding ("proibido usar/dar bid no termo '<nome do produto>'" — Search continua liberado normalmente, só não pode usar o nome da marca em keyword ou copy; adicione o termo como negativa exata). Leia a cláusula inteira antes de classificar: se ela cita o nome do produto dentro da frase de restrição, é caso (b), não (a). Sem a página confirmada com fonte, trate o canal como "não verificado" e não gaste 1 centavo em Search. (Caso real, corrigido em 2026-07-27: a cláusula 10a da FemiCore foi lida errado como bloqueio total de Search — na verdade é caso (b), só proíbe o termo "FemiCore". YuSleep e Lymph Tonic são exemplos verificados de caso (a), bloqueio total do canal.)
